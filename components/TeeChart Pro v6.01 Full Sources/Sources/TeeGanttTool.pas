...
procedure TGanttTool.ChartMouseEvent(AEvent: TChartMouseEvent;
  ...
  begin
    ...
    begin
      ...
      FXOriginal:=ParentChart.BottomAxis.CalcPosPoint(X);

      // Check bar under mouse ( to resize at start or end )
      for t:=0 to Gantt.Count-1 do
      begin
        tmp:=ParentChart.LeftAxis.CalcPosValue(Gantt.YValues[t]);

        if (Y>=tmp-Half) and (Y<=tmp+Half) then
        begin
          tmp1:=ParentChart.BottomAxis.CalcPosValue(Gantt.StartValues[t]);
          tmp2:=ParentChart.BottomAxis.CalcPosValue(Gantt.EndValues[t]);
  ...
  begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            tmp:=ParentChart.BottomAxis.CalcPosValue(Gantt.StartValues[t]);
            ...
            begin
              tmp:=ParentChart.BottomAxis.CalcPosValue(Gantt.EndValues[t]);
    ...
    begin
      with ParentChart.BottomAxis do
        Change:=CalcPosPoint(X)-FXOriginal;
