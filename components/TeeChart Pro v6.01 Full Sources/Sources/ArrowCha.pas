...
Type
  ...
  protected
    Procedure AddSampleValues(NumValues:Integer); override; { <-- to add random arrow values }
...
Procedure TArrowSeries.AddSampleValues(NumValues:Integer);
Var tmpDifX : Integer;
    tmpDifY : Integer;
    t       : Integer;
    tmpX0   : Double;
...
Begin
  ...
  begin
    ...
    Begin
      tmpX0:=tmpX+System.Random(tmpDifX);
      tmpY0:=MinY+System.Random(tmpDifY);
      AddArrow( tmpX0,tmpY0,
                tmpX0+System.Random(tmpDifX),  { X1 }
                tmpY0+System.Random(tmpDifY)   { Y1 }
                );
    end;
  end;
end;
...
Begin  { This overrided method is the main paint for Arrow points. }
  ...
  With ParentChart do
  begin
    if View3D then TPointerAccess(Pointer).PrepareCanvas(Canvas,tmpColor)
...
initialization
  RegisterTeeSeries(TArrowSeries,@TeeMsg_GalleryArrow,@TeeMsg_GalleryExtended,2);
end.
