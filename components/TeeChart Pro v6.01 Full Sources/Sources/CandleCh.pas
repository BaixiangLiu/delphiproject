...
Uses Math, SysUtils, TeeProcs, TeeProCo, TeeConst;
...
Begin
  ...
  TPointerAccess(Pointer).PrepareCanvas(ParentChart.Canvas,clTeeColor);
...
var tmpY : Integer;
Begin
  PrepareCanvas(ValueIndex=FirstValueIndex, ValueColor[ValueIndex]);
...
begin
  ...
  Begin
    AddXY(tmpX,System.Random(Round(DifY) div 15));
    tmpX:=tmpX+StepX;
  end;
end;
...
initialization
  ...
  RegisterTeeSeries( TVolumeSeries, @TeeMsg_GalleryVolume,
                     @TeeMsg_GalleryFinancial, 1);
  RegisterTeeFunction( TADXFunction, @TeeMsg_FunctionADX, @TeeMsg_GalleryFinancial );
  RegisterTeeFunction( TRSIFunction, @TeeMsg_FunctionRSI, @TeeMsg_GalleryFinancial );
finalization
