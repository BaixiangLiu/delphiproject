...
Uses TeeConst, TeePenDlg, TeeProCo;

{ create a Form to edit the Series Text source }
Procedure TeeEditSeriesTextSource(ASource:TSeriesTextSource);
var tmp : TSeriesTextEditor;
begin
  tmp:=TSeriesTextEditor(TeeCreateForm(TSeriesTextEditor,ASource.Owner));
  with tmp do
  try
...
initialization
  ...
  {$IFNDEF TEEOCX}
  TeeSources.Add(TSeriesTextSource);
  {$ENDIF}
finalization
  {$IFNDEF TEEOCX}
  TeeSources.Remove(TSeriesTextSource);
  {$ENDIF}
end.
