...
begin
  c:=nil;
  result:=Add(AStyle,ACaption,vaNull,AData,c);
end;

Function TInspectorItems.Add( AStyle:TInspectorItemStyle; Const ACaption:String;
                               AData:TObject; const AOnChange:TNotifyEvent):TInspectorItem;
begin
  result:=Add(AStyle,ACaption,vaNull,AData,AOnChange);
end;

Function TInspectorItems.Add( AStyle:TInspectorItemStyle; Const ACaption:String;
                               const AOnChange:TNotifyEvent):TInspectorItem;
begin
  result:=Add(AStyle,ACaption,vaNull,nil,AOnChange);
end;
  ...
  begin
    ...
    begin
      if {$IFDEF D5}(not Items[t].IsLine){$ELSE}(Items[t].Caption<>'-'){$ENDIF}
         and (Items[t].Enabled)
         and (Items[t].Tag<>5) then { 5 means "dont put at inspector" }
      begin
        ...
        begin
          Caption:=tmpSt;
          Data:=TObject(AMenu.Items[t].HelpContext);
          IData:=AMenu.Items[t];
    ...
    if Assigned(tmp) then
      ...
      begin
        if tmp.IData is TMenuItem then
           result:=(tmp.IData as TMenuItem).Tag
      end
...
begin { draw checkboxes and buttons at Property Inspector }
  ...
  begin
    ...
    begin
      if CurrentItem<>nil then Brush.Color:=CurrentItem.HelpContext
                          else Brush.Color:=Item(ARow).Value;
    ...
    begin
      ...
      if (CellType(ARow)<>-1) and (CurrentItemCount=0) and
         ...
         (Item(ARow).Style<>iiInteger) then
      begin { button }
        {$IFDEF CLX}
        Pen.Style:=psSolid;
        FillRect(ARect);
      ...
      case CellType(ARow) of
        ...
        3: begin { pen }
             ...
             begin
               TeeSetTeePen(Pen,tmpPen,tmpPen.Color);
       ...
       4: begin { gradient }
            DrawEllipsis;
            tmpGrad:=TChartGradient(Item(ARow).Data);
       ...
       7: begin { font }
            DrawEllipsis;
            with TTeeFont(Item(ARow).Data) do
            begin
              Font.Color:=Color;
              Font.Name:=Name;
              Font.Style:=Style;
       ...
       9: begin { pattern brush }
            ...
            tmpBrush:=TChartBrush(Item(ARow).Data);
...
begin { execute the inspector property (menu item associated) }
  ...
  begin
    ...
    begin
      ...
      if (tmpItem.Style=iiString) or (tmpItem.Style=iiInteger) then
      begin
        with IEditGrid do
        begin
      ...
      begin
        ...
        begin
          ...
          iiBoolean: begin
                       if tmpItem.Value then tmpItem.Value:=False
                                        else tmpItem.Value:=True;
                     end;
            iiColor: tmpItem.Value:=EditColor(Self,tmpItem.Value);
