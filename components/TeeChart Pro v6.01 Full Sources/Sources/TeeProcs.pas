...
type
  ...
  TZoomPanning=packed class(TPersistent)
  private
    FActive     : Boolean;
  public
    X0,Y0,X1,Y1 : Integer;
  ...
  protected
    GLComponent    : TComponent; { internal }
...
Begin
  ...
  begin
    if BackImageMode=pbmStretch then
    begin
      if Z>0 then
    ...
    begin
	 ...
	 if Z=0 then Canvas.ClipRectangle(Rect)
      ...
      if BackImageMode=pbmTile then TileBitmap
      else { draw centered }
      Begin
	FillPanelRect(Rect);
...
begin
  ...
  begin
    ...
    if (FShadow.Size<>0) and (FBrush.Style<>bsClear) then
...
begin
  result:=ITeeEventListener(List^[Index]);
...
type TCanvas3DAccess=class(TTeeCanvas3D);
...
begin
  ...
  try
    ...
    Panel.Canvas.FontZoom:=OldFontZoom;
  end;
...
Const MaxDefaultColors=20;
      DefaultPalette:Array[1..MaxDefaultColors] of TColor=
        ( clRed,
          clGreen,
          clYellow,
          clBlue,
...
var t:Integer;
begin
  SetLength(ColorPalette,MaxDefaultColors+1);
  for t:=1 to MaxDefaultColors do
      ColorPalette[t]:=DefaultPalette[t];
end;
