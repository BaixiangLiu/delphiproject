...
begin
  ...
  begin
    ...
    if Assigned(TTeePanelAccess(FPanel).Listeners) then { 5.01 }
...
Procedure TTeeCommander.DoMouseMove(X,Y:Integer);
   ...
   Procedure DoRotate;
    ...
    begin
      ...
      With FPanel,View3DOptions do
      begin
        tmp:=Round(10.0*(FOldY-Y)/TeeDistance(ChartWidth,ChartHeight));
        tmp2:=Round(tmp*Zoom/100.0);
        if tmp>0 then Zoom:=Zoom+Math.Max(1,tmp2)
                 else Zoom:=Zoom+Math.Min(-1,tmp2);
