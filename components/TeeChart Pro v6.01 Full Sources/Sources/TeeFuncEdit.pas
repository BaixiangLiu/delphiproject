    ...
    for t:=0 to Count-1 do
      ...
      if Assigned(FunctionClass) and
         ...
         (IndexOfObject(TObject(FunctionClass))=-1) then
             AddObject(ReplaceChar(Description^,TeeLineSeparator,' '),
...
begin
  ...
  if Assigned(TheSeries) and Assigned(TheSeries.ParentChart) then
       TTeePanelAccess(TheSeries.ParentChart).Listeners.Add(Self);
...
begin
  if Assigned(TheSeries) and Assigned(TheSeries.ParentChart) then
       TTeePanelAccess(TheSeries.ParentChart).RemoveListener(Self);
  inherited;
end;
